...
const TrackTile: React.FC<ITrackTileProps> = ({ track }) => {
    ...
    return (
        <div className="flex flex-col justify-start items-start w-[180px]">
            <div className="relative flex items-center justify-center group">
                <img src={imageSrc.concat(track.image)} alt={track.name} className="w-[180px] h-[180px] object-cover rounded-[16px]"/>
                <Button className="absolute invisible group-hover:visible" onClick={() => playTracks([track])}>
                    <PlayerPlay className=" w-[54px] h-[54px]"/>
                </Button>
            </div>
            <div className="flex flex-col w-full">
                <span className="font-roboto font-medium text-white text-xl truncate pt-[15px]">{track.name}</span>
